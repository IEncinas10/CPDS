...
In \textit{timey} algorithm the writes temptatives are allocated in a ordered queue by the timestamp, being only discarded the transactions with older timestamp than the current version of the object (last read or written object).
