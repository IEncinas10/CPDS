...
num_entries=(1 2 3 4 5 6 7 8 11 21 31)
...
#for clients in "${num_clients[@]}"; do
    ...
    #erl -noshell -pa ebin -eval "timey:start($clients, $d_entries, $d_reads, $d_writes, $maxtime)" > numclients/$filename & pid=$!; sleep $sleeptime; kill $pid
...
#for clients in "${num_clients[@]}"; do
    #echo $clients, $j, $k, $d;
    #filename=opty"$clients".out;
    #erl -noshell -pa ebin -eval "timey:start($clients, $d_entries, $d_reads, $d_writes, $maxtime)" > numclients2/$filename & pid=$!; sleep $sleeptime; kill $pid
    #geomean=$(grep "Mean" numclients2/$filename | awk -F '[:]' '{print $2}')
    #stddev=$(grep "Stddev" numclients2/$filename | awk -F '[:]' '{print $2}')
    #echo $clients, $geomean, $stddev >> numclients2/clean
#done

#for entries in "${num_entries[@]}"; do
    #echo $entries, $j, $k, $d;
    #filename=opty"$entries".out;
    #erl -noshell -pa ebin -eval "timey:start($d_clients, $entries, $d_reads, $d_writes, $maxtime)" > numentries/$filename & pid=$!; sleep $sleeptime; kill $pid
    #geomean=$(grep "Mean" numentries/$filename | awk -F '[:]' '{print $2}')
    #stddev=$(grep "Stddev" numentries/$filename | awk -F '[:]' '{print $2}')
    #echo $entries, $geomean, $stddev >> numentries/clean
#done

#for entries in "${num_entries[@]}"; do
    #echo $entries, $j, $k, $d;
    #filename=opty"$entries".out;
    #erl -noshell -pa ebin -eval "timey:start($d_clients, $entries, 3, 3, $maxtime)" > numentries2/$filename & pid=$!; sleep $sleeptime; kill $pid
    #geomean=$(grep "Mean" numentries2/$filename | awk -F '[:]' '{print $2}')
    #stddev=$(grep "Stddev" numentries2/$filename | awk -F '[:]' '{print $2}')
    #echo $entries, $geomean, $stddev >> numentries2/clean
#done

for entries in "${num_entries[@]}"; do
    echo $entries, $j, $k, $d;
    filename=opty"$entries".out;
    erl -noshell -pa ebin -eval "timey:start(100, $entries, 1, 1, $maxtime)" > numentries3/$filename & pid=$!; sleep $sleeptime; kill $pid
...
#for reads in "${num_reads[@]}"; do
    ...
    #erl -noshell -pa ebin -eval "timey:start($d_clients, $d_entries, $reads, $d_writes, $maxtime)" > numreads/$filename & pid=$!; sleep $sleeptime; kill $pid
...
#for writes in "${num_writes[@]}"; do
    ...
    #erl -noshell -pa ebin -eval "timey:start($d_clients, $d_entries, $d_reads, $writes, $maxtime)" > numwrites/$filename & pid=$!; sleep $sleeptime; kill $pid
...
#for ratio in "${read_ratios[@]}"; do
    ...
    #erl -noshell -pa ebin -eval "timey:start(10, $d_entries, $reads, $writes, $maxtime)" > readratio/$filename & pid=$!; sleep $sleeptime; kill $pid
...
#for percentage in "${sub_percentage[@]}"; do
    ...
    #export subset_percentage=$percentage; erl -noshell -pa ebin -eval "timey:start(100, $d_entries, 0, 2, $maxtime)" > subset2/$filename & pid=$!; sleep $sleeptime; kill $pid
