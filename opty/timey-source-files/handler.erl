...
handler(Client, Tref, Time, Store, UnvalRd, ValRd, UnvalWr, TentativeWr) ->	
    receive
        ...
        {replyrd, {ok, Value}, Ref} ->
            ...
            handler(Client, Tref, Time, Store, NewUnvalRd, [Entry|ValRd], UnvalWr, TentativeWr);

        {write, Ref, N, Value} ->
            Entry = store:lookup(N, Store),
            Entry ! {write, Ref, Time, Value, self()},
            handler(Client, Tref, Time, Store, UnvalRd, ValRd, [{Ref, N, Entry}|UnvalWr], TentativeWr);
        ...
        {reply, abort, _Ref} ->
            lists:foreach(fun(Pid) -> Pid ! {abort, Time} end, ValRd),
