...
char **argv;
{
	...
	/* Make sure we do use row xlocal[0] before it is received */
	...
	/* S17: nreq=2, synchronize with r[0], r[1] */
	...
	/* S18: Wait for Isend(xlocal[1], ...), whose state is stored at r[3] */
	...
	/* S19: send local diffnorm, store state in r[0] */
    ...
    /* Collect into x the data segments distributed (xlocal) and print x.
       Hint: each process sends its local segment (not the ghost areas)
	     to process 0. */
    MPI_Igather(xlocal[1], maxn * (maxn / size), MPI_DOUBLE, x, maxn * (maxn / size), MPI_DOUBLE, 0, MPI_COMM_WORLD,
		&r[0]); /* Statement S20 */
    /* S20: specify sendbuff, xlocal[1] and r[0], which will store Igather's state*/
