...
char **argv;
{
    ...
    MPI_Sendrecv(xlocal[maxn / size], maxn, MPI_DOUBLE, next_nbr, 0, xlocal[0], maxn, MPI_DOUBLE, prev_nbr, 0,
    ...
    MPI_Sendrecv(xlocal[1], maxn, MPI_DOUBLE, prev_nbr, 1, xlocal[maxn / size + 1], maxn, MPI_DOUBLE, next_nbr, 1,
		 MPI_COMM_WORLD, &status); /* Statement S12 */

    /* S12: specify number of elements to send (size of the row) and datatype for both send and receive. Also, specify
     * the communicator. Change status for &status */
