...
char **argv;
{
    ...
    /* S7: specify sendbuff and number of elements to send */

    /* Receive from previous unless I'm the first process (rank==0)
       and store row in initial row (ghost area) */
    if (rank > 0)
	MPI_Irecv(xlocal, maxn, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD, &r[nreq++]); /* Statement S8 */
    /* S8: specify recvbuff and number of elements to receive */
    ...
    /* S9: specify sendbuff(first owned row) and number of elements to send */
    ...
    /*
     * S10: specify recv(last owned row) and number of elements to send. Also, specify which element of the MPI_Request
     * r array must hold the communication information.
     *
     * The nreq++ here is important because we'll use it in MPI_Waitall. As we know the information beforehand
     * I would use its known size instead of relying on this '++', it seems kind of error prone.
     */
    ...
    /* S11: specify sendbuff(local error) and recvbuff(total error) for the reduce. */
