...
char **argv;
{
    ...
    next_nbr = rank + 1;
    if (next_nbr >= size)
	next_nbr = MPI_PROC_NULL; /* Statement S13 */
    /* S13: set next_nbr to MPI_PROC_NULL if we're the last process. */
	...
	/* 
	 * S14: Combine the diffnorms from every process. We need allreduce because every process needs
	 * the total error in order to know whether to stop or not
	 */
    ...
    /* 
     * S15: specify number of elements to send (size of rows * nº of rows)
     * datatype of send type
     *
     * datatype of recv type
