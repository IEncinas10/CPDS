    ...
    if (myid == 0) {
      printf("I am the master (%d) and going to distribute work to %d additional workers ...\n", myid, numprocs-1);
        ...
        switch( param.algorithm ) {
            case 0: // JACOBI
                    //MPI_Send(&param.u[np*(rowsWorkers-1)], np, MPI_DOUBLE, myid+1, 0, MPI_COMM_WORLD);
                    //MPI_Recv(&param.u[np*rowsWorkers], np, MPI_DOUBLE, myid+1, 0,  MPI_COMM_WORLD, &status);
                    //printf("MAGIC\n");
                    residual = relax_jacobi(param.u, param.uhelp, np, rowsWorkers);
                // Copy uhelp into u
                for (int i=0; i<rowsWorkers+2; i++)
                        for (int j=0; j<np; j++)
        ...
        for(int i = 1; i < numprocs; i++){
            MPI_Recv(&param.u[np*rowsWorkers*i], np*(rowsWorkers+2), MPI_DOUBLE, i, i, MPI_COMM_WORLD, &status);
        }
        ...
        switch( algorithm ) {
            case 0: // JACOBI
                    ...
                    //if(myid != numprocs - 1){
                        ...
                        //MPI_Recv(&u[(rows+1*(columns+2)) * (columns + 2)], columns+2, MPI_DOUBLE, myid+1, 0, MPI_COMM_WORLD, &status);
                    //}
        ...
        MPI_Send(&u[0], (columns + 2)*(rows + 2), MPI_DOUBLE, 0, myid, MPI_COMM_WORLD);
