...
\begin{itemize}
	\item Speedup evaluation and report: We have introduced tables with the speedup obtained for every Parallel Model and size of the problem. Also, a brief comment of the resulting values have been added to the section.
	\item Modification of NB / for loops on Jacobi: We have modified the structure of the for loops. The 4 loops have been reduced to 2 for loops. This change deletes the blocks that uses NB so we don't need to assign any value depending the number of threads to it.
